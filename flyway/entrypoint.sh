...
DATABASE_VERSION=$(PGPASSWORD=gnosis psql --tuples-only -U postgres -h postgres.web3signer-gnosis.dappnode -p 5432 -d web3signer-gnosis -c "SELECT version FROM database_version WHERE id=1;" | awk '{print $1}' | tr -d '[:space:]')
